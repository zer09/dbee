	...
	Describe("dbee tests", func() {
		for _, eng := range engines {
			...
			Context("Read indexed values on partition", func() {
				...
				It("will store and read string", func() {
					var err error
					...
					for _, t := range tests {
						ss0, _ := part.Get()
						ss0.Wstring("istring", t.in)
						ss0.Commit()

						ss0, _ = part.Get(ss0.ID())
						Expect(ss0.Rstring("istring")).Should(BeEquivalentTo(t.out))

						ss0, err = part.One("istring", t.in)
						Expect(err).NotTo(HaveOccurred())
						Expect(ss0).ShouldNot(BeNil())
						Expect(ss0.Rstring("istring")).Should(BeEquivalentTo(t.out))
					}
				})
			})
